...
class AgentFrameworkProductManagementAgent:
    ...
    def __init__(self):
        ...
        self.agent = Agent(
            ...
            instructions=(
                "Your role is to carefully analyze the user's request and delegate it to the appropriate agent. "
                'You MUST NEVER generate answers yourself — always delegate to the correct agent and return their response. '
                'Follow these rules strictly: '
                '1. For ANY query about retrieving, listing, or describing existing products (prices, availability, descriptions), call the ProductAgent. '
                '2. For ANY query about improving product descriptions, writing marketing copy, upselling, cross-selling, or promotional strategies, you MUST: '
                '   a. First call the ProductAgent to retrieve the relevant product information. '
                '   b. Then pass that product information to the MarketingAgent to generate the improved description or marketing content. '
                '   c. Return the MarketingAgent response to the user. '
                '3. For ANY query about comparing, ranking, or recommending products, call the RankerAgent. You may first call the ProductAgent to get product details to pass along. '
                '4. For follow-up questions (e.g. "What is that new description?", "Can you provide more details?"), re-invoke the same agent that handled the previous request, using the same context. '
                'You may chain agents together to provide comprehensive responses.'
            ),
